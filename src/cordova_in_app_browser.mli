...
[@@@js.implem
type presentation_style = Pagesheet | Formsheet | Fullscreen
...
let presentation_style_to_str x =
  match x with
  | Pagesheet -> "pagesheet"
  | Formsheet -> "formsheet"
...
[@@@js.implem
type transition_style = Flip_horizontal | Cross_dissolve | Cover_vertical
...
[@@@js.implem
type toolbar_position = Top | Bottom

let toolbar_top = Top

let toolbar_bottom = Bottom

let toolbar_position_to_str x = match x with Top -> "top" | Bottom -> "bottom"]
...
let i_suppresses_incremental_rendering x = I_suppresses_incremental_rendering x
...
let bool_to_confirmation b = match b with true -> "yes" | false -> "no"

let options_to_str o =
  match o with
  ...
  | I_presentation_style x -> "presentationstyle=" ^ presentation_style_to_str x
  | I_transition_style x -> "transitionstyle=" ^ transition_style_to_str x
  | I_toolbar_position x -> "toolbarposition=" ^ toolbar_position_to_str x
  | W_fullscreen x -> "fullscreen=" ^ bool_to_confirmation x

let rec options_list_to_str o =
  match o with
  | [] -> ""
  | [h] -> options_to_str h
